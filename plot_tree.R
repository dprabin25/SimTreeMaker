...
# Usage: Rscript plot_tree.R <nwk_path> <out_dir> <stem> <model_name> <model_type> <mode>
# Requires: ape

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 5) {
  cat("Usage: Rscript plot_tree.R <nwk_path> <out_dir> <stem> <model_name> <model_type> <mode>\n")
  quit(status = 1)
}
...
model_name <- args[4]
model_type <- args[5]
mode       <- if (length(args) >= 6) args[6] else "Lineage"
